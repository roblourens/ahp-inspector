...
export async function openViewer(context: vscode.ExtensionContext): Promise<vscode.WebviewPanel> {
  ...
  if (candidate) {
    ...
    try {
      await sessions.open({ path: candidate.fsPath });
    } catch {
      console.warn("AHP Inspector could not open the active log; showing the picker instead.");
    }
  }
