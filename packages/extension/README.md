...
VS Code extension that opens the AHP Log Viewer inside a webview. The same
...
- **AHP Log Viewer: Open** (`ahpViewer.open`) — opens or focuses the viewer
...
```sh
pnpm install
pnpm -F @ahp-viewer/ui build         # produces packages/ui/dist
pnpm -F @ahp-viewer/extension build  # produces dist/extension.cjs + ui-dist/
```
...
`AHP Log Viewer: Open` command from the command palette.

## Build output

- `dist/extension.cjs` — bundled extension entrypoint (CommonJS, `vscode`
  external, `@ahp-viewer/*` packages bundled in).
