...
// Spawns ahp-viewer via tsx with a mini fixture, asserts UI-SPEC §10 success
...
describe("ahp-viewer CLI launch (Plan 02-05)", () => {
  ...
  it("prints §10 success copy and exits cleanly on SIGTERM", async () => {
    ...
    const successLine = await waitForLine(r, /AHP Log Viewer running at http:\/\/127\.0\.0\.1:\d+/);
    expect(successLine).toMatch(/^AHP Log Viewer running at http:\/\/127\.0\.0\.1:\d+$/);
  ...
  it("respects --no-open (no browser line beyond the §10 copy)", async () => {
    const r = spawnCliRaw([CLI_MINI, "--port", "0", "--no-open"]);
    lastChild = r.child;
    await waitForLine(r, /AHP Log Viewer running at http:\/\/127\.0\.0\.1:\d+/);
