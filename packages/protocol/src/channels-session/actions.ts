 ...
 *
 * - Client-dispatched values. A client mutates one or more properties. Only
 *   properties with `sessionMutable: true` in the schema may be changed. The
 *   server validates and broadcasts the action; the reducer merges the new
 *   values into `state.config.values`. Clients MUST NOT populate `schema`.
 * - Server-emitted schema. When the server resolves the dynamic configuration
 *   schema for a session, it broadcasts an action carrying the new `schema`.
 *   The action MAY also carry refined `config` values in the same emission,
 *   applied atomically.
 *
 * @category Session Actions
 * @version 1
...
export interface SessionConfigChangedAction {
  ...
  /** Updated config values. When omitted, `state.config.values` is left unchanged. */
  config?: Record<string, unknown>;
  /** When `true`, replaces all config values instead of merging. */
  replace?: boolean;
  /** New session configuration schema. When present, fully replaces `state.config.schema`. */
 ...
 * returns to `idle`.
 *
 * Common use-case: truncate old data then call `startTurn` with an edited
 * message.
 *
 * @category Session Actions
 * @version 1
