...
function refineToolCallContributor(
  ...
  if (next.kind === ToolCallContributorKind.Client) {
    log?.(
      `Ignoring late client contributor '${next.clientId}' because client execution ownership must be established at tool call start`,
    );
    return current;
  }
  return next;
...
function hasOpenInputRequest(state: ChatState): boolean {
  return (
    state.activeTurn?.responseParts.some(
      (part) => part.kind === ResponsePartKind.InputRequest && part.response === undefined,
    ) ?? false
  );
}
...
function upsertInputRequestPart(
  state: ChatState,
  request: InputRequestResponsePart["request"],
): ChatState {
    ...
    case ActionType.ChatToolCallReady:
      return refreshSummaryStatus(
        updateToolCallInParts(state, action.turnId, action.toolCallId, (tc) => {
          ...
          const toolInput =
            action.toolInput ?? (tc.status === ToolCallStatus.Streaming ? undefined : tc.toolInput);
          if (action.confirmed) {
            return {
      ...
      return refreshSummaryStatus(
        updateToolCallInParts(state, action.turnId, action.toolCallId, (tc) => {
          if (
            tc.status !== ToolCallStatus.Running &&
            tc.status !== ToolCallStatus.PendingConfirmation &&
            tc.status !== ToolCallStatus.AuthRequired
          ) {
            return tc;
          }
          ...
          const base = tcBaseWithMeta(tc, action._meta);
          const confirmed =
            tc.status === ToolCallStatus.Running || tc.status === ToolCallStatus.AuthRequired
              ? tc.confirmed
