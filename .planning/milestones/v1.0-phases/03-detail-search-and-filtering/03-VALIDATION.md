...
| **Quick run command** | `pnpm -F @ahp-viewer/<pkg> test` for changed package |
...
| 03-W3-01 | Facets | 3 | SEARCH-02, SEARCH-03, SEARCH-04 | T-03-01 | Facet selectors are deterministic and non-blocking under 50k rows | jsdom/perf | `pnpm -F @ahp-viewer/ui test src/state/selectors.test.ts src/state/selectors.perf.test.ts` | ❌ W3 | ⬜ pending |
| 03-W4-01 | Search API/UI | 4 | SEARCH-01, SEARCH-03 | T-03-01, T-03-02, T-03-08 | Substring-only query, 256-char query cap, 5000 result cap, abortable requests | integration/jsdom | `pnpm -F @ahp-viewer/server test src/search-routes.test.ts && pnpm -F @ahp-viewer/ui test src/transport/search-client.test.ts` | ❌ W4 | ⬜ pending |
| 03-W5-01 | Grouping | 5 | TIME-05, EVENT-06 | T-03-05 | Group headers preserve virtualization; serverSeq gaps/auth failures surfaced | jsdom/unit | `pnpm -F @ahp-viewer/ui test src/components/timeline/grouping.test.tsx && pnpm -F @ahp-viewer/core test src/row-projection.test.ts` | ❌ W5 | ⬜ pending |
