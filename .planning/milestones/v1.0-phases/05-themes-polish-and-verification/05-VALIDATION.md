...
| **Quick run command** | `pnpm -F @ahp-inspector/ui test` |
...
- **After every plan wave:** Run `pnpm -F @ahp-inspector/ui test` and any added E2E smoke relevant to the wave.
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 05-W0-token-guard | 05-00 | 0 | THEME-02 | T-05-01 | Components cannot introduce raw color literals outside tokens. | static/unit | `pnpm -F @ahp-inspector/ui test src/styles/no-hex-in-components.test.ts` | yes | green |
| 05-W0-token-complete | 05-00 | 0 | THEME-01, THEME-02, THEME-03 | T-05-02 | Dark/light/hacker define the required semantic token set. | unit/static | `pnpm -F @ahp-inspector/ui test src/styles/theme-tokens.test.ts` | yes | green |
...
| 05-layout | 05-01 | 1 | THEME-05 | T-05-04 | Detail panel does not crush the timeline at laptop widths; ultra-wide remains balanced. | unit/e2e/UAT | `pnpm -F @ahp-inspector/ui test src/components/shell/AppShell.test.tsx` plus E2E viewport test | yes | green |
| 05-theme-polish | 05-02 | 2 | THEME-01, THEME-02, THEME-03 | T-05-05 | Theme colors/effects are token-driven, accessible, and reduced-motion aware. | unit/browser/UAT | `pnpm -F @ahp-inspector/ui test src/components/shell/HeaderBar.test.tsx src/styles/*.test.ts` | yes | green |
| 05-persistence | 05-03 | 3 | THEME-04 | T-05-06 | Theme and viewer prefs persist without storing log content, paths, prompts, or raw payloads. | unit/e2e | `pnpm -F @ahp-inspector/ui test src/persistence/persist-effect.test.ts src/state/persistence.test.ts src/components/shell/HeaderBar.test.tsx` | yes | green |
| 05-ui-coverage | 05-04 | 4 | VERIFY-02 | T-05-07 | UI flows are covered at app/integration level, not only isolated components. | component/integration | `pnpm -F @ahp-inspector/ui test` | yes | green |
