...
- `pnpm sync:ahp` regenerates `@ahp-viewer/protocol` from sibling `../agent-host-protocol/types`.
...
- Parser imports canonical `ActionEnvelope` and `ProtocolNotification` from `@ahp-viewer/protocol`.
- `@ahp-viewer/shared/ahp` is retained only as a compatibility alias layer.
...
No blocking gaps found. Phase 7 can build deterministic replay against `@ahp-viewer/protocol`.
