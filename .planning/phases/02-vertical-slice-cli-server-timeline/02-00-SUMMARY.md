...
provides:
  ...
  - Phase-2 dependency allow-list (react, react-dom, vite, @vitejs/plugin-react, @tanstack/react-virtual, zustand, lucide-react, tailwindcss, @tailwindcss/vite, jsdom, @testing-library/{react,user-event,jest-dom}, open, @types/react, @types/react-dom, @ahp-viewer/ui)
  - "no CDN URLs in UI source" guardrail covering packages/ui/src and packages/ui/index.html
  - @ahp-viewer/ui workspace package (Vite 8 + React 19 + Tailwind 4 + jsdom Vitest) with smoke test
  - Vendored Inter Variable + JetBrains Mono Variable woff2 with OFL LICENSE files
  - Locked EventRow projection contract + projectRow() pure function exported from @ahp-viewer/core
...
tech-stack:
  ...
  patterns:
    ...
    - "Pure projection contracts published from @ahp-viewer/core for both server projector and UI consumers"
...
key-decisions:
  - "EventRow contract owned by @ahp-viewer/core (portable, no DOM/Node imports) so server projector and UI row component cannot drift"
...
**React/Vite legalised under packages/ui only, @ahp-viewer/ui scaffolded with vendored OFL fonts, and the locked EventRow projection contract published from @ahp-viewer/core for Wave-1 server + timeline consumers.**
...
- @ahp-viewer/ui workspace package builds (`pnpm -F @ahp-viewer/ui build` → 190.5 kB JS) and runs Vitest in jsdom (smoke test green).
...
- `EventRow` interface, `projectRow()`, and helper functions (`bandFor`, `dirGlyphFor`, `kindTagFor`, `actionFamilyFor`, `formatTs`, `payloadPreviewOf`) exported from `@ahp-viewer/core`; 42 unit tests cover all enum branches, latency-band boundaries (49/50/199/200/999/1000), parse-error fallback, and field formatting.
...
2. **Task 2: Scaffold @ahp-viewer/ui package + vendor fonts** — `9f95182` (feat)
3. **Task 3: Publish EventRow projection contract from @ahp-viewer/core** — `8f52d80` (feat)
...
- `test/security.test.ts` — extended ALLOW set with Phase-2 deps + `@ahp-viewer/ui`; added "no CDN URLs in UI source" describe block with comment-stripper
...
- `packages/ui/src/App.tsx` — placeholder rendering `<div data-testid="app-root">AHP Log Viewer</div>`
...
- **Verification:** `pnpm -F @ahp-viewer/ui exec tsc --noEmit` → 0 errors; `pnpm typecheck` → all 7 packages green.
...
- **Fix:** Used `tsc --noEmit` (matches sibling packages like `@ahp-viewer/core`).
...
- `pnpm install && pnpm -F @ahp-viewer/ui build` → emits `dist/index.html` + 190.5 kB JS bundle
- `pnpm -F @ahp-viewer/ui test` → smoke test green in jsdom
...
- FOUND: test/security.test.ts (modified, contains @ahp-viewer/ui + @tanstack/react-virtual + CDN-URL test)
