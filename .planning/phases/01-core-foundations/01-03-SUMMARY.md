...
dependency_graph:
  requires:
    - "@ahp-viewer/shared (Plan 01-02)"
    - "@ahp-viewer/parser (Plan 01-02)"
    - "test/fixtures/tiny.jsonl (Plan 01-01)"
  provides:
    - "@ahp-viewer/core: EventStore + Correlator + Status"
    - "@ahp-viewer/host-node: NodeHostAdapter, TailReader, NodeLogHandle, discoverVsCodeLogs (stub)"
    - "@ahp-viewer/server: startHealthServer (loopback-only Hono)"
    - "@ahp-viewer/cli: ahp-viewer entrypoint wiring host → parser → store/correlator → server"
  affects:
...
key_files:
  ...
  modified:
    ...
    - "packages/host-node/package.json (+ @ahp-viewer/shared)"
...
| 3 | 3a1d15e | feat | 01-03 | Hono health server (127.0.0.1) + ahp-viewer CLI                  |
...
`packages/cli/src/index.ts` — `ahp-viewer [file] [--port <n>] [--no-server]`. Boots `NodeHostAdapter` + `EventStore` + `Correlator`, optionally starts `startHealthServer`, then for the supplied file: tails bytes → `LineSplitter` → `parseLine` → `normalize`/`makeParseErrorEvent` → `store.append`. SIGINT/SIGTERM dispose the watcher and close the server before `process.exit(0)`.
...
[ahp-viewer] listening on http://127.0.0.1:5173
[ahp-viewer] opened /Users/.../ahp-viewer/test/fixtures/tiny.jsonl (560 bytes)
^C
...
**1. [Rule 3 — Blocker] `host-node` lacked `@ahp-viewer/shared` dependency**
- **Found during:** Task 2 typecheck — `import type { HostAdapter } from "@ahp-viewer/shared"` in the new `host-adapter.ts` failed module resolution.
- **Fix:** Added `@ahp-viewer/shared: workspace:*` to `packages/host-node/package.json`. Re-ran `pnpm install`; lockfile updated.
- **Files modified:** `packages/host-node/package.json`, `pnpm-lock.yaml`.
- **Commit:** 73ac272.

**2. [Rule 3 — Blocker] CLI lacked `@ahp-viewer/{core, parser, shared}` dependencies**
- **Found during:** Task 3.
